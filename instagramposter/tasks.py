...
class MarketingAnalysisTasks:
    def product_analysis(self, agent, product_website, product_details):
        return Task(
            ...
            expected_output="A detailed analysis of the product website.",
            agent=agent
        )
    ...
    def competitor_analysis(self, agent, product_website, product_details):
        return Task(
            description=dedent(
                ...
                """),
            expected_output="A detailed analysis of the product's competitors.",
            agent=agent
        )
    ...
    def campaign_development(self, agent, product_website, product_details):
        return Task(
            description=dedent(
                ...
                """),
            expected_output="A detailed marketing campaign plan.",
            agent=agent
        )
    ...
    def instagram_ad_copy(self, agent):
        return Task(
            description=dedent(
                f"""Craft an engaging Instagram post copy.
                ...
                """),
            expected_output="3 engaging Instagram ad copies.",
            agent=agent
        )
    ...
    def take_photograph_task(self, agent, copy, product_website, product_details):
        return Task(
            description=dedent(
                f"""You are working on a new campaign for a super important customer,
                ...
                """),
            expected_output="3 options of photographs with descriptions.",
            agent=agent
        )
    ...
    def review_photo(self, agent, product_website, product_details):
        return Task(
            description=dedent(
                f"""Review the photos you got from the senior photographer.
...
class quality_assurance_tasks_class:
    def quality_assurance(self, agent, product_website, product_details, task_output):
        return Task(
            description=dedent(
                f"""Perform a quality assurance check on the task request: make a blog post for a website.
                the website is {product_website}
                the further more details are: {product_details}
                ...
                if the output of the task is irrelavant to the task, you should say "TASK FAILED".
                Here are the output of the task that you are going to check: {task_output}
                """),
            expected_output="A detailed QA report on the task or 'TASK FAILED'.",
            agent=agent
        )
